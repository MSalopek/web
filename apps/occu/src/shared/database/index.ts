...
export interface SerializedBlockDetails {
...
class Pindexer {
  ...
  // occu related queries
  async getBlockDetails(height: number): Promise<Selectable<BlockDetails> | undefined> {
    return this.db
  ...
  async getBlockTimestamp(height: number): Promise<Date | null | undefined> {
  ...
  async getBlockTimestamps(heights: number[]): Promise<SerializedBlockDetails[]> {
    const result = await this.db
      .selectFrom('block_details')
      .select(['height', 'timestamp', 'root'])
      .where(
        'height',
        'in',
  ...
  // assumed to be denominated in USDC
  async getPenumbraPriceAtHeight(height: number): Promise<number | null | undefined> {
    const result = await this.db
