...
type TaxableTransactionType = 'all' | 'income' | 'expense' | 'disposal';

// All transactions, regardless of their tax classification
export const FilterableTransactionList = observer(
  ({ taxType = 'all' }: { taxType: TaxableTransactionType }) => {
    ...
    const filteredTransactions = transactions?.pages.map(page =>
      page.filter(tx => {
        switch (taxType) {
          case 'income': {
            const { type } = classifyTransaction(tx.view);
            return IncomeTaxActions.has(type);
          }
          case 'disposal': {
            const { type } = classifyTransaction(tx.view);
            return DisposalTaxActions.has(type);
          }
          // all transactions are 'expense' type because fees were paid
          default:
...
// You can then simplify your existing components to use this shared component:
export const AllTransactions = observer(() => <FilterableTransactionList taxType='all' />);
export const IncomeTransactions = observer(() => <FilterableTransactionList taxType='income' />);
export const ExpensesTransactions = observer(() => <FilterableTransactionList taxType='expense' />);
export const DisposalsTransactions = observer(() => (
  <FilterableTransactionList taxType='disposal' />
));
